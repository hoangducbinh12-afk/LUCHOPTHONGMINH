...
# --- 1. CẤU HÌNH GIAO DIỆN MOBILE & LIGHT MODE ---
st.set_page_config(page_title="TUAN PHONG V10.2 MOBILE", layout="wide")
st.markdown("""
    <style>
    /* CSS tự thích ứng màn hình */
    .main-box { 
        ...
        /* Cỡ chữ tự co giãn theo chiều rộng màn hình */
    ...
    /* Tối ưu các thẻ chỉ số trên Mobile */
    ...
    /* Ép các bảng dài có thanh cuộn ngang trên điện thoại */
    .stTable {
    ...
    /* Thu nhỏ khoảng cách Sidebar trên mobile */
    section[data-testid="stSidebar"] > div {
        padding-top: 2rem;
    }
    
    /* Style bảng đanh thép */
...
@st.cache_data
def calculate_master_v102(last_gdb, raw_107, weights, pts_vtri_str, pts_tt_str):
...
# --- 3. GIAO DIỆN CHÍNH ---
st.title("🛡️ TUAN PHONG MASTER V10.2")

with st.sidebar:
    st.header("⚙️ QUẢN LÝ")
    up_json = st.file_uploader("Nạp Lịch sử (.json):", type="json")
    ...
    if img_file:
        ...
        if st.button("🚀 QUÉT & PHÂN TÍCH AI", type="primary", use_container_width=True):
            ...
            if gdb_n:
                db = st.session_state.db
                if db["last_gdb"]:
                    df_old = calculate_master_v102(db["last_gdb"], db["raw_107"], tuple(db["weights"]), json.dumps(db["pts_vi_tri"]), json.dumps(db["pts_thuoc_tinh"]))
...
if st.session_state.db["last_gdb"]:
    ...
    df_m = calculate_master_v102(db["last_gdb"], db["raw_107"], tuple(active_w), json.dumps(db["pts_vi_tri"]), json.dumps(db["pts_thuoc_tinh"]))
    
    t1, t2, t3 = st.tabs(["🎯 DÀN AI", "📊 ĐỐI TRỌNG", "🕒 NHẬT KÝ"])
    
    with t1:
        # Sử dụng columns nhưng Streamlit sẽ tự chồng hàng trên Mobile
        c1, c2 = st.columns(2)
        with c1:
            st.metric("Kỳ hiện tại", db['last_gdb'])
            avg_r = np.mean([h['Rank_AI'] for h in db['history'][:3]]) if db['history'] else 50
            st.metric("Chỉ số cầu", f"{round(avg_r,1)} Rank")
        with c2:
    ...
    with t2:
        st.subheader("⚖️ Đối trọng Engine (%)")
        w_df = pd.DataFrame({"App": [f"A{i}" for i in range(1,7)], "W%": [round(x,1) for x in active_w]})
        st.dataframe(w_df.set_index("App").T, use_container_width=True)

    with t3:
        if db["history"]:
            # Bảng nhật ký sẽ có thanh cuộn ngang tự động nhờ CSS ở đầu
            st.table(pd.DataFrame(db["history"])[['Kỳ', 'GĐB', 'Số', 'Rank_AI', 'Rank_A1', 'Rank_A2', 'Rank_A3', 'Rank_A4', 'Rank_A5', 'Rank_A6', 'Time']])

with st.sidebar:
    st.divider(); st.download_button("💾 SAO LƯU", json.dumps(st.session_state.db), "LUC_HOP_MOBILE.json", use_container_width=True)
