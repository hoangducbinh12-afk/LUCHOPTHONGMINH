...
# --- 1. CẤU HÌNH GIAO DIỆN SÁNG & CHỮ NHỎ ---
st.set_page_config(page_title="TUAN PHONG V10.5", layout="wide")
st.markdown("""
    <style>
    .main-box { 
    ...
    [data-testid="stMetric"] { background: #f8fafc; padding: 10px; border-radius: 8px; border: 1px solid #e2e8f0; }
...
@st.cache_data
def calculate_master_v105(last_gdb, raw_107, weights, pts_vtri_str, pts_tt_str):
...
# --- 3. GIAO DIỆN CHÍNH ---
st.title("🛡️ TUAN PHONG MASTER AI V10.5")

# TẠO KHU VỰC NẠP FILE NGAY TRÊN ĐẦU
with st.expander("📂 NHẤN ĐỂ NẠP / SAO LƯU DỮ LIỆU (.JSON)", expanded=not st.session_state.db["last_gdb"]):
    c_up, c_down = st.columns(2)
    with c_up:
        up_json = st.file_uploader("Chọn file .json đã lưu:", type="json")
        if up_json: st.session_state.db = json.load(up_json); st.rerun()
    with c_down:
        st.write("Lưu lại trí khôn AI:")
        st.download_button("💾 TẢI FILE BACKUP", json.dumps(st.session_state.db), "LUC_HOP_BACKUP.json")

# SIDEBAR CHO QUÉT ẢNH
with st.sidebar:
    ...
    if img_file:
        ...
        if st.button("🚀 QUÉT & CẬP NHẬT", type="primary"):
            ...
            if gdb_n:
                ...
                if db["last_gdb"]:
                    df_old = calculate_master_v105(db["last_gdb"], db["raw_107"], tuple(db["weights"]), json.dumps(db["pts_vi_tri"]), json.dumps(db["pts_thuoc_tinh"]))
...
if st.session_state.db["last_gdb"]:
    db = st.session_state.db
    df_m = calculate_master_v105(db["last_gdb"], db["raw_107"], tuple(db["weights"]), json.dumps(db["pts_vi_tri"]), json.dumps(db["pts_thuoc_tinh"]))
    ...
    with t2: st.json(db["pts_thuoc_tinh"])
    with t3:
        if db["history"]: st.table(pd.DataFrame(db["history"])[['Kỳ', 'GĐB', 'Số', 'Rank_A1', 'Rank_A2', 'Rank_A3', 'Rank_A4', 'Rank_A5', 'Rank_A6', 'Time']])
