...
# --- 1. STYLE CHỮ NHỎ & GIAO DIỆN SÁNG ---
st.set_page_config(page_title="TUAN PHONG V10.6 FULL", layout="wide")
st.markdown("""
    <style>
    .main-box { 
        background-color: #ffffff; color: #334155; padding: 12px; border-radius: 10px; 
        font-family: 'JetBrains Mono', monospace; font-size: clamp(0.8rem, 3vw, 1.1rem); 
...
@st.cache_data
def calculate_master_v106(last_gdb, raw_107, weights, pts_vtri_str, pts_tt_str):
...
# --- 3. GIAO DIỆN ---
st.title("🛡️ TUAN PHONG MASTER AI V10.6")
    ...
    if img_file:
        ...
        if st.button("🚀 QUÉT & CẬP NHẬT", type="primary"):
            ...
            if gdb_n:
                ...
                if db["last_gdb"]:
                    df_old = calculate_master_v106(db["last_gdb"], db["raw_107"], tuple(db["weights"]), json.dumps(db["pts_vi_tri"]), json.dumps(db["pts_thuoc_tinh"]))
...
if st.session_state.db["last_gdb"]:
    db = st.session_state.db
    df_m = calculate_master_v106(db["last_gdb"], db["raw_107"], tuple(db["weights"]), json.dumps(db["pts_vi_tri"]), json.dumps(db["pts_thuoc_tinh"]))
    
    t1, t2, t3, t4 = st.tabs(["🎯 DÀN AI", "📊 ĐỐI TRỌNG & NẠP DATA", "🕒 NHẬT KÝ", "🛠 CHI TIẾT APP 5"])
    ...
    with t2:
        st.subheader("⚖️ Bảng Đối trọng AI (%)")
        w_df = pd.DataFrame({"Engine": [f"App {i}" for i in range(1,7)], "Tỷ trọng (%)": [round(x,1) for x in db["weights"]]})
        st.table(w_df.set_index("Engine").T)
        st.line_chart(db["weights"])
        
        st.divider()
        st.subheader("📂 QUẢN LÝ DỮ LIỆU (.JSON)")
        c_up, c_down = st.columns(2)
        with c_up:
            up_json = st.file_uploader("Nạp file 300 kỳ:", type="json")
            if up_json: st.session_state.db = json.load(up_json); st.success("Nạp thành công!"); st.rerun()
        with c_down:
            st.download_button("💾 SAO LƯU DỮ LIỆU", json.dumps(st.session_state.db), "MASTER_DATA.json")
    ...
    with t4:
        st.subheader("🧠 Điểm Khan Thuộc Tính (App 5)")
        st.write("Dữ liệu này được tích lũy từ 300 kỳ mày đã nạp:")
